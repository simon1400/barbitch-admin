    ...
    {LABEL_COLORS.map((c) => (
      <button
        ...
        className={`h-5 w-5 rounded-full border-2 transition ${
          value === c ? 'scale-110 border-gray-700 dark:border-gray-500' : 'border-transparent hover:scale-105'
        }`}
        style={{ background: c }}
        title={c}
      />
      ...
      <div className="space-y-4">
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {labels.map((l) => (
              <div key={l.documentId} className="flex items-center gap-2 rounded-md border border-gray-200 dark:border-[#2e2e2c] bg-white dark:bg-[#2a2a28] px-2.5 py-1.5">
                <input
                  className="min-w-0 flex-1 rounded border border-transparent px-1.5 py-0.5 text-sm text-gray-800 dark:text-gray-300 hover:border-gray-200 focus:border-gray-300 focus:outline-none"
                ...
                <button
                  ...
                  className="ml-1 text-gray-400 dark:text-gray-500 hover:text-red-600 disabled:opacity-40"
                  title="Smazat štítek"
                >
                  ✕
                </button>
            ...
            {labels.length === 0 && <p className="text-sm text-gray-400 dark:text-gray-500">Zatím žádné štítky.</p>}
          </div>
        )}

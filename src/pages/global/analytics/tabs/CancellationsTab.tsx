      ...
      {error || !data ? (
        ...
        <>
          <div className="flex gap-4 flex-wrap mb-6">
            ...
            <div className="bg-white rounded-lg shadow-sm px-4 py-3">
              <div className="text-xs text-ink-faint">Состоявшихся визитов</div>
              <div className="text-2xl font-bold text-ink">{data.totalVisits}</div>
              <div className="text-xs text-ink-faint mt-1">
                доля no-show:{' '}
          ...
          <StatSection title="По мастерам" id="cancel-masters" defaultOpen>
            {data.byMaster.length === 0 ? (
              <div className="text-ink-soft py-8 text-center">За месяц отмен нет.</div>
            ) : (
              <TableWrapper>
          ...
          <StatSection title="Клиенты с отменами (топ-15)" id="cancel-clients">
            {data.topClients.length === 0 ? (
              <div className="text-ink-soft py-8 text-center">Нет данных.</div>
            ) : (
              <TableWrapper>
                <table className="w-full text-left min-w-[620px]">
                  <thead>
              ...
              <table className="w-full text-left min-w-[620px]">
                ...
                <tbody>
                  {data.byWeekday.map((d) => (
                    <tr key={d.label} className="hover:bg-surface-hover transition-colors">

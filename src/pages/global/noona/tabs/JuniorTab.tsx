...
export default function JuniorTab() {
  ...
  return (
    <>
      <h3 className="text-2xl font-bold text-gray-800 mb-2">Junior služby</h3>
      <p className="text-sm text-gray-500 mb-6">
        Генерирует junior-копии Noona event_types с ценой -{JUNIOR_DISCOUNT_PERCENT}% и сохраняет
        маппинг senior → junior в Strapi.
      </p>
      ...
      {(status === 'ready' || status === 'done' || status === 'generating') && (
        <>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm mb-4 grid gap-4 md:grid-cols-2">
            ...
            <label className="flex flex-col gap-1">
              <span className="text-xs font-semibold text-gray-600">Цель (junior категория)</span>
              <select
                  ...
                  <tbody>
                    {results.map((r) => (
                      <tr key={r.plan.key} className="border-b last:border-b-0">
                        ...
                        <td className="py-2 text-right tabular-nums">{r.plan.juniorPrice} Kč</td>
                        <td className="py-2 text-xs font-mono text-gray-600">{r.juniorId ?? '—'}</td>
                        <td className="py-2">
                          {r.status === 'ok' && (
          ...
          <div className="mt-6 p-4 bg-blue-50 rounded-lg text-xs text-blue-700">
            ...
            <ul className="list-disc pl-5 space-y-1">
              <li>
                Скрипт создаёт копию каждой выбранной услуги в Noona под целевой категорией с ценой
                -{JUNIOR_DISCOUNT_PERCENT}%.
              </li>
              <li>
                Если senior — combo (hidden=true), junior-копия тоже hidden. Top-level услуги
                остаются видимыми.
              </li>
              <li>
                После генерации в Noona вручную назначь junior-мастеру (Yana) право работать с junior
                event_types через её Skills.
              </li>
              <li>
                Маппинг senior → junior сохраняется в Strapi (service-junior-map). Клиентский сайт
                читает его и автоматически подменяет event_type ID при выборе junior-мастера.
              </li>
              <li>Кнопка можно нажимать многократно — уже созданные пары не дублируются.</li>
            </ul>
          </div>
        </>
      )}

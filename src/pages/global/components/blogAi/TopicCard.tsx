  ...
  return (
    <div className="bg-white rounded-xl border border-line p-5 shadow-sm">
      ...
      <div className="flex items-start justify-between gap-2 mb-2">
        <h3 className="font-bold text-ink text-sm leading-snug">{topic.title}</h3>
      ...
      {/* Description */}
      <p className="text-[12.5px] leading-[1.55] font-medium text-ink-hint mb-3">{topic.description}</p>

      {/* Keywords */}
      {topic.keywords?.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-3">
          {topic.keywords.map((kw, i) => (
            <span key={i} className="px-1.5 py-0.5 bg-line-soft text-ink-soft text-[10px] rounded">
              {kw}
            </span>
          ))}
        </div>
      ...
      <div className="text-[11px] text-ink-faint mb-3">
        {topic.scheduledDate && <span className={'block mb-2'}>Date: <span className={'inline-block px-1.5 py-0.5 bg-green-100 text-ink-soft rounded'}>{formatDate(topic.scheduledDate)}</span></span>}
        ...
        {topic.internalLinks?.length > 0 && (
          <div className={'flex gap-3'}>
            <span>Links:</span>
            <div>
              {topic.internalLinks.map(item => <div className={'mb-1'}><span className={'inline-block px-1.5 py-0.5 bg-info-bg text-ink-soft rounded'}>{item}</span></div>)}
            </div>
          </div>
        )}
      </div>
      ...
      {/* Error */}
      {error && (
        <p className="text-xs text-neg mb-2">{error}</p>
      )}
        ...
        {topic.stage === 'proposed' && (
          <>
            ...
            <button
              ...
              disabled={loading}
              className="px-3 py-1.5 bg-line-soft text-ink-muted text-xs font-medium rounded-lg hover:bg-gray-200 disabled:opacity-50"
            >
              Zamítnout
            </button>
          </>
